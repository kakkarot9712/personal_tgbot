...
use crate::database::{schema::Person, traits::CollectionHelpers};

pub async fn handle_person_asked(
    q: CallbackQuery,
    bot: Bot,
    settle_diag: super::DialogueWithState,
    db: Arc<Database>,
) -> ResponseResult<()> {
    ...
    if let Some(Message { id, chat, .. }) = q.message {
        ...
        settle_diag
            .update(super::State::AmountAsked { person })
            .await
            .unwrap();
    ...
    match settle_balance {
        Ok(b) => {
            if b > person.balance || b < 0.0 {
                ...
                .await
                .unwrap();
                settle_diag.update(super::State::Idle).await.unwrap();
